// Package appauth carries caller credentials through gRPC metadata. Phase ③
// dual-stack window: the trusted tenant identity (x-tenant-key, injected by
// the portal proxy — internal-network trust) rides alongside the legacy
// per-app credentials (x-app-key / x-app-secret); Resolve applies the D-③1
// precedence (tenant key authoritative, legacy pair only when the trusted
// key is absent). Verification happens in the service layer — not an
// interceptor — so module-mode in-process callers share the same path as
// gRPC clients. The legacy half is deleted wholesale when the window closes
// (phase ④).
package appauth
...
// existing incoming keys (legacy credentials) survive — and are IGNORED at
// Resolve time per D-③1 when the trusted key is present.
